...
use super::{gradient_dot2_fancy, gradient_dot3, hash_primes2, hash_primes3, impl_noise23, mask, masked_add, masked_sub, nmasked_add, nmasked_sub, nmul_add, primes, select};
...
fn gen2([x, y]: [f32; 2], seed: i32, _: OpenSimplex2s) -> f32 {
    ...
    let mut a1 = mul_add(2.0 * (1.0 + 2.0 * G2) * (1.0 / G2 + 2.0), t, a + (-2.0 * (1.0 + 2.0 * G2) * (1.0 + 2.0 * G2)));
    a1 *= a1;
    a1 *= a1;
...
fn gen3([x, y, z]: [f32; 3], mut seed: i32, _: OpenSimplex2s) -> f32 {
    ...
    loop {
        ...
        a1 *= a1;
        a1 *= a1;
        let h1 = hash_primes3(seed, nmasked_add(xrbp, primes::X, flip1), masked_add(yrbp, primes::Y, flip1), masked_add(zrbp, primes::Z, flip1));
        ...
        a2 *= a2;
        a2 *= a2;
        let h2 = hash_primes3(seed, masked_add(xrbp, primes::X, flip2), nmasked_add(yrbp, primes::Y, flip2), masked_add(zrbp, primes::Z, flip2));
        ...
        a3 *= a3;
        a3 *= a3;
        let h3 = hash_primes3(seed, masked_add(xrbp, primes::X, flip3), masked_add(yrbp, primes::Y, flip3), nmasked_add(zrbp, primes::Z, flip3));
