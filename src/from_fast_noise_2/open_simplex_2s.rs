use crate::{fast_max, from_fast_noise_2::gradient_dot2_fancy};

use super::{gradient_dot3, hash_primes2, hash_primes3, noise, primes};
...
fn gen2([x, y]: [f32; 2], seed: i32) -> f32 {
    ...
    let forward_x = xsi.mul_add(-2.0, ysi) < boundary_xy;
    let forward_y = ysi.mul_add(-2.0, xsi) < boundary_xy;
    ...
    let mut a1 = (2.0f32 * (1.0 + 2.0 * G2) * (1.0 / G2 + 2.0)).mul_add(t, a + (-2.0 * (1.0 + 2.0 * G2) * (1.0 + 2.0 * G2)));
    a1 *= a1;
    a1 *= a1;
    let value = a1.mul_add(v1, value);
